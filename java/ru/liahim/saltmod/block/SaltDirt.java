...
public class SaltDirt extends Block {
	...
	public void updateTick(World world, BlockPos pos, IBlockState state, Random rand)
	{
		if (!world.isRemote)
		{
			...
			{
				//Таяние льда
				if (world.getBlockState(pos.up()).getBlock().getMaterial() == Material.craftedSnow ||
    				world.getBlockState(pos.up()).getBlock().getMaterial() == Material.ice)
				{
	...
	public MapColor getMapColor(IBlockState state)
    {
    	...
    	if (state.getBlock().getMetaFromState(state) == 1) {color = MapColor.quartzColor;}
        return color;
    }
