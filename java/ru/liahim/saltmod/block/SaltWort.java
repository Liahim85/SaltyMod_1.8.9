...
public class SaltWort extends BlockBush implements IGrowable {
	...
	public boolean canBlockStay(World world, BlockPos pos, IBlockState state)
    {
		...
		if ((B.getMaterial() == Material.grass || (B.getMaterial() == Material.ground && B != ModBlocks.saltDirt) ||
		    ...
		    (B == ModBlocks.saltDirt && B.getMetaFromState(state2) == 0)) && World.doesBlockHaveSolidTopSurface(world, pos.down()))
		{
			return true;
		}
			...
			if ((soil == ModBlocks.saltDirt && meta2 == 0) || (soil == ModBlocks.saltDirtLite && (meta2 == 1 || meta2 == 2)))
			{
				if (rand.nextInt(SaltConfig.saltWortGrowSpeed) == 0)
				{
					...
					else if (meta == 1 && world.getLightFromNeighbors(pos) >= 12)
					{
						...
						{world.setBlockState(pos2, ModBlocks.saltDirtLite.getStateFromMeta(2), 3);}
						else if (soil == ModBlocks.saltDirtLite && meta2 == 2)
						{world.setBlockState(pos2, ModBlocks.saltDirtLite.getStateFromMeta(1), 3);}
						...
						if (S < 7)
						{
							...
							for (int zj = z - 1; zj <= z + 1; zj++) {
								  ...
								  ((soil == ModBlocks.saltDirtLite && (meta2 == 1 || meta2 == 2)) || (soil == ModBlocks.saltDirt && meta2 == 0)))
								{
									...
									{world.setBlockState(jPos, ModBlocks.saltDirtLite.getStateFromMeta(2), 3);}
									else if (jSoil == ModBlocks.saltDirtLite && jMeta == 2)
									{world.setBlockState(jPos, ModBlocks.saltDirtLite.getStateFromMeta(1), 3);}
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumFacing side, float hitX, float hitY, float hitZ)
    {
		...
		if (!world.isRemote && world.getBlockState(pos).getBlock().getMetaFromState(state) == 4 &&
				 player.getCurrentEquippedItem() != null && player.getCurrentEquippedItem().getItem() == Items.shears)
		{
			world.setBlockState(pos, this.getStateFromMeta(2), 3);
			...
			world.playSoundEffect(pos.getX() + 0.5D, pos.getY() + 1.0D, pos.getZ() + 0.5D, "mob.sheep.shear", 1.0F, 1.2F);
	...
	public static boolean fertilize(World world, BlockPos pos)
	{
		...
		for (int z3 = z - 1; z3 <= z + 1; z3++) {
			...
			if (jBlock == ModBlocks.saltWort && jMeta < 4)
			{
				...
				{
					if (jMeta == 0) {world.setBlockState(jPos, ModBlocks.saltWort.getStateFromMeta(1), 3); chek = true; P = true;}

					else
					{
						if (jMeta == 1)
						{world.setBlockState(jPos, ModBlocks.saltWort.getStateFromMeta(2), 3); chek = true; P = true;}
						else if (jMeta == 2)
						{world.setBlockState(jPos, ModBlocks.saltWort.getStateFromMeta(3), 3); chek = true; P = true;}
						else if (jMeta == 3)
						{world.setBlockState(jPos, ModBlocks.saltWort.getStateFromMeta(4), 3); chek = true; P = true;}

						if (jMeta < 5)
						{
							if (jSoil == ModBlocks.saltDirt)
							{world.setBlockState(jPos.down(), ModBlocks.saltDirtLite.getStateFromMeta(2), 3);}
							else if (jSoil == ModBlocks.saltDirtLite && jMeta2 == 2)
							{world.setBlockState(jPos.down(), ModBlocks.saltDirtLite.getStateFromMeta(1), 3);}
				...
				else if (jMeta == 0) {world.setBlockState(jPos, ModBlocks.saltWort.getStateFromMeta(1), 3); chek = true; P = true;}
			}
			
			if (P) {P = false; if (!world.isRemote) world.playAuxSFX(2005, jPos, 0);}
		}
		}
