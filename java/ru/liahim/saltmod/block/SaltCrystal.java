...
public class SaltCrystal extends BlockBush {
	...
	{
		if (world.provider.getDimensionId() == 0 && pos.getY() < 40) {player.addStat(AchievSalt.saltCrystal, 1);}
	}
	...
	public void onEntityCollidedWithBlock(World world, BlockPos pos, IBlockState state, Entity entity)
    {
		if (!world.isRemote){
			    ...
			    EntityList.getEntityString(entity).toLowerCase().contains("witch")))
			{
				...
				world.playSoundEffect(pos.getX() + 0.5D, pos.getY() + 1.0D, pos.getZ() + 0.5D, "dig.glass", 1.0F, 2.0F);
