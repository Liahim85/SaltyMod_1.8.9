...
public class DispenserBehaviorSaltPinch implements IBehaviorDispenseItem {
	...
	public ItemStack dispense(IBlockSource source, ItemStack stack)
	{
		...
		Block blockFase = stateFace.getBlock();
		...
		if (blockFase == ModBlocks.saltDirtLite)
		{
			...
			if (meta == 0 || meta > 2)
			{world.setBlockState(posFace, blockFase.getStateFromMeta(1), 3); stack.splitStack(1); chek = true;}
			else if (meta == 1)
			{world.setBlockState(posFace, blockFase.getStateFromMeta(2), 3); stack.splitStack(1); chek = true;}
			else if (meta == 2)
			{world.setBlockState(posFace, ModBlocks.saltDirt.getDefaultState(), 3); stack.splitStack(1); chek = true;}	
		}
		
		else if (soil == ModBlocks.saltDirtLite && (world.isAirBlock(posFace) || blockFase == ModBlocks.saltWort))
		{
			int meta = world.getBlockState(posSoil).getBlock().getMetaFromState(stateSoil);

			if (meta == 0 || meta > 2)
			{world.setBlockState(posSoil, soil.getStateFromMeta(1), 3); stack.splitStack(1); chek = true;}
			else if (meta == 1)
			{world.setBlockState(posSoil, soil.getStateFromMeta(2), 3); stack.splitStack(1); chek = true;}
			else if (meta == 2)
			{world.setBlockState(posSoil, ModBlocks.saltDirt.getDefaultState(), 3); stack.splitStack(1); chek = true;}
		}
		
		else if (blockFase == ModBlocks.saltDirt) {}
		else if (soil == ModBlocks.saltDirt && (world.isAirBlock(posFace) || blockFase == ModBlocks.saltWort)) {}		
		else {BehaviorDefaultDispenseItem.doDispense(world, stack.splitStack(1), 1, enumfacing, iposition); chek = true;}
	
		if (chek) {source.getWorld().playAuxSFX(1000, source.getBlockPos(), 0);}
